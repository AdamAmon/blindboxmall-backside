...
export class PayService {
  ...
  // 创建充值订单并生成支付链接
  async createRechargeOrder(userId: number, amount: number): Promise<{ record: Recharge, payUrl: string }> {
    try {
      ...
      try {
        ...
        console.error('[调试] createRechargeOrder alipaySdk 初始化异常:', sdkErr);
        throw new MidwayHttpError('支付宝SDK初始化失败: ' + sdkErr.message, 500);
      }
      const bizContent = {
  ...
  async getRechargeRecords(userId: number) {
    try {
      const result = await this.rechargeModel.find({ where: { recharge_user_id: userId }, order: { recharge_id: 'DESC' } });
      return result;
    } catch (err) {
  ...
  async handleAlipayNotify(rawBody: string) {
    ...
    // 查找订单
    const record = await this.rechargeModel.findOne({ where: { recharge_out_trade_no: params.out_trade_no } });
    if (!record) {
      console.error('[调试] handleAlipayNotify 订单不存在:', params.out_trade_no);
    ...
    if (user) {
      ...
      console.error('[调试] handleAlipayNotify 用户不存在:', record.recharge_user_id);
    }
