...
describe('地址模块 service 层单元测试', () => {
  ...
  afterEach(async () => {
    // 清理该用户所有地址
    await userService.addressModel.delete({ user_id: userId });
  });

  it('should create address for user', async () => {
    const address = await userService.createAddress(userId, TEST_ADDRESS);
  ...
  it('should throw if recipient is missing', async () => {
    ...
    await expect(userService.createAddress(userId, invalid as any)).rejects.toThrow();
  });

  it('should return empty array for new user', async () => {
    const list = await userService.listAddresses(userId);
    expect(Array.isArray(list)).toBe(true);
    expect(list.length).toBe(0);
  });

  it('should return addresses after creation', async () => {
    await userService.createAddress(userId, TEST_ADDRESS);
    const list = await userService.listAddresses(userId);
  ...
  it('should update address fields', async () => {
    const address = await userService.createAddress(userId, TEST_ADDRESS);
    const updated = await userService.updateAddress(userId, {
      ...address,
      recipient: '李四',
      phone: '13987654321'
    });
  ...
  it('should set address as default', async () => {
    await userService.createAddress(userId, { ...TEST_ADDRESS, is_default: false });
    const addr2 = await userService.createAddress(userId, { ...TEST_ADDRESS, detail: '测试路2号', is_default: false });
    await userService.setDefaultAddress(userId, addr2.id);
    const list = await userService.listAddresses(userId);
  ...
  it('should delete address by id', async () => {
    const address = await userService.createAddress(userId, TEST_ADDRESS);
    const result = await userService.deleteAddress(userId, address.id);
  ...
  it('should not throw when deleting non-existent address', async () => {
    await expect(userService.deleteAddress(userId, 999999)).rejects.toThrow('地址不存在');
  });
});